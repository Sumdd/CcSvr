            ...
            try
            {
                ...
                else
                {
                    ...
                    if (m_mRecord.T_PhoneNum.Length > 5)
                    {
                        ...
                        if (m_uContinue != 1)
                        {
                            Log.Instance.Warn($"[CenoSocket][m_cIp][m_fDial][{m_uAgentID} inrule callee:{m_mRecord.T_PhoneNum},way:{m_uContinue}]");
                            m_cIp.m_fIpDialSend(m_pWebSocket, m_sUUID, -1, "Err内呼规则");
                            return;
                        }
                    }
                    #endregion
